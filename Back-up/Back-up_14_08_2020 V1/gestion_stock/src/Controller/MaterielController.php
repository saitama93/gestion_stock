...
class MaterielController extends AbstractController
{
     ...
     * Permet de télécharger le modèle de CSV pour les matériels
     */
    ...
    /**
     * @Route("stock/multiButton",name = "Materiel.multiButton",methods={"GET","POST"})
     * Permet de gérer la modification ou la suppression de plusieurs matériels
     */
    public function multiButton(Request $request){
        if (isset($_POST['delete'])){
            $keys = array_keys($_POST);
            $temp = '';
            foreach ($keys as $key){
                if ($key != 'delete' && $key != 'tablestock_length'){
                    $key = trim($key,'materiel');
                    $temp .= $key.',';
                }
            }
            $session = $request->getSession();
            $session->set('materiel',$temp);
            return $this->redirectToRoute('Materiel.deleteMultiple') ;
        } else if (isset($_POST['update'])) {
            $keys = array_keys($_POST);
            $temp = '';
            foreach ($keys as $key){
                if ($key != 'update' && $key != 'tablestock_length' && $key != 'checkAll'){
                    $key = trim($key,'materiel');
                    $temp .= $key.',';
                }
            }
            $session = $request->getSession();
            $session->set('materiel',$temp);
            return $this->redirectToRoute('Materiel.updateMultiple') ;
        }
        else {
            return $this->redirectToRoute('Materiel.index');
        }
    ...
    /**
     * @Route("stock/updateMultiple",name="Materiel.updateMultiple",methods={"GET","POST"})
     * Permet de mettre à jour les plusieurs matériels
     */
    public function updateMultiple(Request $request){
        $em = $this->getDoctrine()->getManager();
        if ($request->isMethod('POST')){
            $session = $request->getSession();
            $temp = $session->get('materiel');
            $keys = explode(',',$temp);
            foreach ($keys as $key){
                if ($key != ''){
                    $materiel = $em->getRepository(Materiel::class)->find($key);
                    if (isset($_POST['statut'])){
                        $statut = $em->getRepository(Statut::class)->find($_POST['statut']);
                        $materiel->setIdstatut($statut);
                    }
                    ...
                    if (isset($_POST['specificite'])){
                        $specificite = $em->getRepository(Specificite::class)->find($_POST['specificite']);
                        $materiel->setIdspecificite($specificite);
                    }
                    if (isset($_POST['type'])){
                        $type = $em->getRepository(Type::class)->find($_POST['type']);
                        $materiel->setIdtype($type);
                    }
                    $em->persist($materiel);
                    $em->flush();
        ...
        $specificites = $em->getRepository(Specificite::class)->findAll();
        return $this->render('stock/updateMultiple.html.twig',[
            'statuts' => $statuts,
            'marques'=>$marques,
            'lieux' => $lieux,
            'types' => $types,
     ...
     * Permet de cacher plusieurs matériels d'un coup
     */
     ...
     * Cache les différents matériels à cacher
     */
