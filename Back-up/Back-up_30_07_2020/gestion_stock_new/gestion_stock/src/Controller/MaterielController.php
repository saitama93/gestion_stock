...
class MaterielController extends AbstractController
{
    ...
    public function edit(Request $request,$id){
        ...
        if ($materiel->getDate() == null || $temp == false){
            $temp = new \DateTime();
        }
    ...
    /**
     * @Route("stock/csv",name="Materiel.downloadCsv",methods={"GET"})
    ...
    public function downloadCsv(){
        ...
        foreach ($materiels as $materiel){
            ...
            $referent = null != $materiel->getIdreferent() ?
                $materiel->getIdreferent()->getIdreferent().'-'.$materiel->getIdreferent()->getNom() : 'NULL';
            $data = array($materiel->getIdmateriel(),
                $materiel->getNumeroserie(),
                $materiel->getStatutLibelle(),
        ...
        if ($request->isMethod('POST')){
            if (isset($_FILES['csvFile'])){
                ...
                if (in_array($file,$extension)){
                     ...
                     */
                    $materiels = $em->getRepository(Materiel::class)->findAll();
                    $chemin='./file/bck'.date('d_m_Y_H_i_s',strtotime('+2 hours')).'.csv';
                    ...
                    foreach ($line as $data){
                        if ($verif){
                            ...
                            $referent = $em->getRepository(Referent::class)->findOneBy(array('idreferent'=>$idRef));
                            switch ($data[2]){
                                case 'Disponible':
                                    $statut = 0;
                                    break;
                            ...
                            $materiel->setIdreferent($referent);
